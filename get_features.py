...
def main(args):
    ...
    min_filename = int(os.path.basename(sorted_paths[0]).split('.')[0])# + 20
    ...
    for j, seq in enumerate(seqs):
        ...
        pca = None
        # 49, 295
        for i, p in tqdm(enumerate(sorted_paths[::3])):
            ...
            try:
                if features.shape[-1] != args.num_dims:
                    ...
                    dyn_mask = retrieve_dyn_mask(int(seq[-1]), int(p.split('/')[-1].split('.')[0]), resize=(512, 288), input_dir=args.root_dir)
    ...
    os.makedirs(f'/data3/zihanwa3/Capstone-DSR/Processing{args.seq}/dinov2features', exist_ok=True)
...
if __name__ == "__main__":
    ...
    args.root_dir = f'/data3/zihanwa3/Capstone-DSR/Processing{args.seq}/?'
